...
- API 名称：Issue Red Invoice - Full Amount (Original Invoice Red Flush Request)
- 产品：税务服务
...
| data.einvoiceApplyList[].hzxxbbh | string | 否 | 红字信息表编号。专票红冲时必填；数电专票红冲时不需要 | `线上未提供` |
...
| data.einvoiceApplyList[].items[].define | object | 否 | 特征值 | `{"Characteristic key":"Example of characteristic value (optional)"}` |
...
`/yonbip/tax/invoiceclient-web/api/invoiceApply/red-withjson?access_token=访问令牌`
...
| message | string | Information | `success` |
| data | object | Response Information | `Operation successful` |
