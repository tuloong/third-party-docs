...
| data.autoAudit | boolean | 否 | Automatic review, meaning invoicing is done directly on the invoice platform without manual confirmation.  <br>false: No automatic review, manual confirmation is required. If not provided, it defaults to true. |
...
| data.einvoiceApplyList[].fpDm | string | 否 | Invoice code, invoice code of the red-ink reversed invoice |
| data.einvoiceApplyList[].fpHm | string | 是 | Invoice Number Invoice Number of the Voided Invoice |
| data.einvoiceApplyList[].hcyy | string | 否 | Red Flush Reason Description: 1 Sales Return 2 Invoice Error 3 Invoice Cancellation 4 Sales Allowance; (Red flush reason is mandatory for digital invoices) Red flush reason is mandatory for digital invoices. When the invoice type is (VAT Normal Invoice, VAT Electronic Normal Invoice, VAT Normal Invoice (Roll), VAT Electronic Normal Invoice (Refined Oil), VAT Normal Invoice (Refined Oil), VAT Normal Invoice (Purchase), Electronic Purchase Invoice, Digital Special Invoice, Digital Normal Invoice), the red flush reason must be provided. |
| data.einvoiceApplyList[].hzxxbbh | string | 否 | Red Letter Information Form Number: Mandatory for special invoice red flush; not required for digital special invoice red flush. |
| data.einvoiceApplyList[].xsfNsrsbh | string | 否 | Seller Taxpayer Identification Number  <br>If a tax number corresponds to only one tax organization (associated invoicing site record), only the seller's tax number needs to be transmitted; either the seller's tax number or the tax organization code must be provided. |
...
| data.auditReturnConfigs[].fpqqlsh | string | 否 | The invoice request serial number should be consistent with the einvoiceApplyList above. |
...
| message | string | 否 | Information |
| data | object | 否 | Response Information |
| data | object | 否 | Response Information |
