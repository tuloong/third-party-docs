...
- 调用 URL：`https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/api/invoiceApply/qr-with-split?access_token={access_token}`
...
```bash
curl -X POST 'https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/api/invoiceApply/qr-with-split?access_token=访问令牌' \
...
1. `xcx`
   ...
   - 必填：否
   - 示例/默认值：`N`
   - 说明：Is it from the Mini Program? Y: Yes; N: No
