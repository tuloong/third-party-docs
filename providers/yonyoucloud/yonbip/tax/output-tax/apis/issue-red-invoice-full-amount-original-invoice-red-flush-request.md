...
- 完整调用地址：https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/red-withjson
...
- 调用 URL：`https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/red-withjson?access_token={access_token}`
...
```bash
curl -X POST 'https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/red-withjson?access_token=访问令牌' \
...
7. `data.einvoiceApplyList.fpDm`
   ...
   - 数组：否
   - 必填：否
   - 说明：Invoice code, invoice code of the red-ink reversed invoice
...
9. `data.einvoiceApplyList.hcyy`
   ...
   - 数组：否
   - 必填：否
   - 说明：Red Flush Reason Description: 1 Sales Return 2 Invoice Error 3 Invoice Cancellation 4 Sales Allowance; (Red
     flush reason is mandatory for digital invoices) Red flush reason is mandatory for digital invoices. When the
     invoice type is (VAT Normal Invoice, VAT Electronic Normal Invoice, VAT Normal Invoice (Roll), VAT Electronic
     Normal Invoice (Refined Oil), VAT Normal Invoice (Refined Oil), VAT Normal Invoice (Purchase), Electronic
     Purchase Invoice, Digital Special Invoice, Digital Normal Invoice), the red flush reason must be provided.
...
11. `data.einvoiceApplyList.xsfNsrsbh`
   ...
   - 说明：Seller Taxpayer Identification Number  
     If a tax number corresponds to only one tax organization (associated invoicing site record), only the seller's
     tax number needs to be transmitted; either the seller's tax number or the tax organization code must be
     provided.
...
29. `data.einvoiceApplyList.items.xmbm`
   - 类型：string
   - 数组：否
   - 必填：否
   - 说明：Project Code

30. `data.einvoiceApplyList.items.xmmc`
   - 类型：string
   - 数组：否
   - 必填：是
   - 说明：Project Name

31. `data.einvoiceApplyList.items.spbm`
   - 类型：string
   - 数组：否
   - 必填：是
   - 说明：Product Code

32. `data.einvoiceApplyList.items.ggxh`
   - 类型：string
   - 数组：否
   - 必填：否
   - 说明：Specification

33. `data.einvoiceApplyList.items.dw`
   - 类型：string
   - 数组：否
   - 必填：否
   - 说明：Unit

34. `data.einvoiceApplyList.items.xmsl`
   - 类型：string
   - 数组：否
   - 必填：否
   - 说明：Item Quantity

35. `data.einvoiceApplyList.items.xmdj`
   - 类型：number
   - 数组：否
   - 必填：否
   - 说明：Item Unit Price

36. `data.einvoiceApplyList.items.xmje`
   - 类型：number
   - 数组：否
   - 必填：否
   - 说明：Item Amount

37. `data.einvoiceApplyList.items.xmjshj`
   - 类型：number
   - 数组：否
   - 必填：否
   - 说明：Item Amount Including Tax

38. `data.einvoiceApplyList.items.sl`
   - 类型：number
   - 数组：否
   - 必填：否
   - 说明：Tax Rate

39. `data.einvoiceApplyList.items.se`
   - 类型：number
   - 数组：否
   - 必填：否
   - 说明：Tax Amount
