...
- 完整调用地址：https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/insertWithArray
...
- 调用 URL：`https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/insertWithArray?access_token={access_token}`
...
```bash
curl -X POST 'https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/insertWithArray?access_token=访问令牌' \
...
1. `FormParam.requestdatas`
   ...
   - 数组：否
   - 必填：是
   - 示例/默认值：`Request parameter details can be found in the Open Platform API information supplementary description - Output Tax Invoice Management - Issuing Blue Invoices: https://fapiao.yonyoucloud.com/apidoc/appendix/`
   - 说明：Blue invoice request parameters, including invoice header and invoice details.
...
2. `FormParam.email`
   ...
   - 说明：Email Push Configuration; [ { "fpqqlsh": "Invoice request serial number must be consistent with the above
     requestdatas, required", "address": "Email address, required" } ]
...
3. `FormParam.sms`
   ...
   - 说明：SMS Push Configuration; [{ "fpqqlsh": "Invoice request serial number must be consistent with the above
     requestdatas, required", "address": "Mobile number, required" }]
