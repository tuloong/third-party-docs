...
- 完整调用地址：https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/issue
...
- 调用 URL：`https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/issue?access_token={access_token}`
...
```bash
curl -X POST 'https://c2.yonyoucloud.com/iuap-api-gateway/yonbip/tax/invoiceclient-web/api/invoiceApply/issue?access_token=访问令牌' \
...
1. `FormParam.requestdatas`
   ...
   - 必填：是
   - 示例/默认值：`[{
	"FPQQLSH": "Invoice request serial number Required",
	"XSF_NSRSBH": "Seller taxpayer identification number Required",
	"JSHJ": "Total amount including tax Required"
}]`
   - 说明：Invoice issuance action parameter
